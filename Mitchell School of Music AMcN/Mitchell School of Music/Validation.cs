...
namespace Mitchell_School_of_Music.Objects
{
    public static class ValidationMethods
    {
        ...
        public static bool validPostcode(Control txt, int min, Control label)
        {
            bool ok = true;
            TextBox txtB = (TextBox)txt;
            Label cbLabel = (Label)label;
            string formatText = cbLabel.Text.Substring(0, cbLabel.Text.Length - 1);
            string strRegex = @"(GI00 0AA)|((([A-Z-[QVX]][0-9][0-9]?)|(([A-Z-[QVX]][A-Z-[IJZ]][0-9][0-9]?)|(([A-Z-[QVX]][0-9][A-HJKPSTUW])|([A-Z-[QVX]][A-Z-[IJZ]][0-9][ABEHMNPRVWXY])))) [0-9][A-Z-[CIKMOV]]{2})";
            ...
            else if (txtB.Text.Length < min)
            {
                ok = false;
                MessageBox.Show(formatText + " must have a length of " + min + "characters.", "Text is too long!", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
